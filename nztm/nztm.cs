...
public class nztm
{
    ...
    private const int NZTM_A = 6378137;
    private const double NZTM_RF = 298.257222101;
    private const double NZTM_CM = 173.0;
    private const double NZTM_OLAT = 0.0;
    private const double NZTM_SF = 0.9996;
    private const double NZTM_FE = 1600000.0;
    private const double NZTM_FN = 10000000.0;
                  ...
                  double ln, double lt, out double ce, out double cn)
    {
        ...
        dlon = ln - cm;
        while (dlon > System.Math.PI) dlon -= TWOPI;
        while (dlon < -System.Math.PI) dlon += TWOPI;
    ...
    static tmprojection get_nztm_projection()
    {
        if (initiallized == 0)
        {
            define_tmprojection(out nztm_projection, NZTM_A, NZTM_RF,
